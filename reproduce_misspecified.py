...
for i in range(num_datasets):
    data = [[]] * 4
    data[0], data[1], data[2], data[3], tau = gen_data(num_samples=dataset_size, linear=True, exponential=True)
...
plt.ylim([0, 1.2])
plt.xlim([0, 5000])
plt.savefig('misspecified.pdf', bbox_inches='tight')
plt.show()
np.save('misspecified_results.npy', np.array(estimate_all))
